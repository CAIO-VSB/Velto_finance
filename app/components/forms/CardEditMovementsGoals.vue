<script lang="ts" setup>
  ...
  const props = defineProps<{
    goalsId?: number | null,
    draft: TGoalsMovements | null
  }>()

  const { getAccountsOnlyActive } = useHttpAccounts()
  const { getAllGoals, updateMovementsGoals } = useHttpGoals()
  ...
  const { mutate, isPending  } = useMutation({
    ...
    mutationFn: (payload: TGoalsMovementsPayload) => updateMovementsGoals(payload.id!, payload),
  ...
  <div class="text-center">
    ...
    >
      <v-dialog v-model="modelValue" max-width="600">
        <v-card rounded="lg" elevation="6" v-if="props.draft">
          <v-card-item class="pa-4 pb-2">
            ...
            <v-card-subtitle class="mt-1">
              Preencha os dados abaixo para editar sua aplicação.
            </v-card-subtitle>
          ...
          <v-card-text >
            ...
            <v-col
              cols="12" md="12" sm="12"
              >
              <v-text-field prepend-inner-icon="mdi-pencil"  prepend-icon="" :rules="nameRules" :counter="45" maxlength="45"  autocomplete="name" name="name" label="Descrição" variant="solo-filled" v-model="props.draft.description"></v-text-field>
            </v-col>

              <v-col
              cols="12" md="12" sm="12"
              >
                <CurrencyInput variant="solo-filled" prepend-inner-icon="mdi-cash" :rules="currencyRules" text-color="primary" autocomplete="off" label="Valor a ser aplicado" hint="Valor que você deseja aplicar" v-model="props.draft.value_paid"/>
              </v-col>

              <v-col
              cols="12" md="12" sm="12"
              >
                <v-date-input prepend-inner-icon="mdi-calendar" prepend-icon="" :rules="dateRules" autocomplete="off" label="Data do lançamento" variant="solo-filled" v-model="props.draft.date_movement"></v-date-input>
              </v-col>
